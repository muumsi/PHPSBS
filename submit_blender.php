...
$config['BLENDER']['job'] = $jobname; 
$config['BLENDER']['folder'] = $target_dir;
$config['BLENDER']['outputfile'] = $OutputFile;
$config['BLENDER']['startframe'] = $startframe;
$config['BLENDER']['endframe'] = $endframe;
$config['BLENDER']['cpu'] = $ncpu;
$config['BLENDER']['mem'] = $memsize;
$config['BLENDER']['maxtime'] = $maxtime;
...
$i=WriteShell($shell_file, $target_dir, $sbatch_stdout, $sbatch_error, $jobname, $ncpu, $memsize, $maxtime, $imedat, $OutputFile, $startframe, $endframe, $nodelist, $nodetype);
...
function WriteShell($shell_file, $target_dir, $sbatch_stdout, $sbatch_error, $jobname, $ncpu, $memsize, $maxtime, $inputfile, $OutputFile, $startframe, $endframe, $nodelist, $nodetype) {
...
$gpu=FALSE;
$test="  " . $nodelist;
if(strpos($test,"gpu")>0 || $nodetype=="CUDA")
{
	$gpu=TRUE;
	$Cycles="GPU";
	$ncpu=4;
...
fwrite($myfile, "# SLURM skript for BLENDER\n");
fwrite($myfile, "\n");
...
if($gpu)
{
	fwrite($myfile, "#SBATCH --partition=gpu\n");
	fwrite($myfile, "#SBATCH --gres=gpu:4\n");
}
fwrite($myfile, "\n");
...
fwrite($myfile, "/ceph/grid/software/blender-2.92.0-linux64/");
fwrite($myfile, "\${PATH:+:\$PATH}\n");

fwrite($myfile, "\n");

// exe
fwrite($myfile, "/ceph/grid/software/blender-2.92.0-linux64/blender");
if($gpu)
{
	fwrite($myfile, " -b -noaudio ");
}
else
{
	fwrite($myfile, " -b ");
...
fwrite($myfile, $inputfile);
fwrite($myfile, " -E CYCLES");
//fwrite($myfile, $rendertype);
fwrite($myfile, " -o //");
fwrite($myfile, $OutputFile);
fwrite($myfile, " -s ");
fwrite($myfile, $startframe);
fwrite($myfile, " -e ");
fwrite($myfile, $endframe);
fwrite($myfile, " -t ");
fwrite($myfile, $ncpu);
if($gpu)
{
	fwrite($myfile, "  -a -- --cycles-device CUDA");
}
else
{
	fwrite($myfile, "  -a -- --cycles-device CPU");
}
fwrite($myfile, "\n");
fwrite($myfile, "\n");
# cleanup
