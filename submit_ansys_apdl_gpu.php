...
$i=WriteShell($shell_file, $target_dir, $sbatch_stdout, $sbatch_error, $jobname, $ncpu, $ngpu, $memsize, $maxtime, $imedat, $exefile, $ansysargs);
...
function WriteShell($shell_file, $target_dir, $sbatch_stdout, $sbatch_error, $jobname, $ncpu, $ngpu, $memsize, $maxtime, $inputfile, $exefile, $ansysargs) {
...
fwrite($myfile, "\n");
fwrite($myfile, "\n");
//fwrite($myfile, "#SBATCH --cpus-per-task=2\n");
fwrite($myfile, "#SBATCH --partition=gpu\n");
fwrite($myfile, "#SBATCH --gres=gpu:");
fwrite($myfile, $ngpu);
fwrite($myfile, "\n");
...
fwrite($myfile, "export LD_LIBRARY_PATH=/ceph/grid/software/ansys_inc/lib64\${LD_LIBRARY_PATH:+:\$LD_LIBRARY_PATH}\n");
fwrite($myfile, "export LD_LIBRARY_PATH=/ceph/grid/software/cuda-10.0\${LD_LIBRARY_PATH:+:\$LD_LIBRARY_PATH}\n");

fwrite($myfile, "export ANSGPU_OVERRIDE=1\n");
fwrite($myfile, "export KMP_AFFINITY=disabled\n");
...
fwrite($myfile, " -p aa_r -dis -mpi openmpi -machines \${mech_hosts%%:} -acc nvidia -na ");
fwrite($myfile, $ngpu);
fwrite($myfile, " -lch -dir \"$target_dir\"");
fwrite($myfile, " -j ");
fwrite($myfile, $jobname);
if(!empty($ansysargs))
{
